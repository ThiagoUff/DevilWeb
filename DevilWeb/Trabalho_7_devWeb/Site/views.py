...
from Site.models import Images, User, Produto,Carrinho
import json
...
def SalvaprodutoCarrinho(request):
    ...
    if form.is_valid():
        print(1)
        produto = get_object_or_404(Produto, pk=form.cleaned_data['produto_id'])
        carrinhos = Carrinho.objects.filter(comprador=request.user,produto = produto)
        if carrinhos:
            carrinho = carrinhos[0]
            carrinho.cotas += form.cleaned_data['produto_qtd']
        else:
            carrinho = Carrinho()
        ...
        result = str(form.cleaned_data['produto_qtd']) + ' Cota de produtos Compradas, total de ' + str(carrinho.cotas)
...
def ListaCarrinho(request):
    carrinhos = Carrinho.objects.filter(comprador=request.user)

    lista_de_carrinhos = []
    for carinho in carrinhos:

        produto = get_object_or_404(Produto, pk=carinho.produto.id)
        lista_de_carrinhos.append({'produto': produto,
                                  'cotas': carinho.cotas,
                                  'preco': carinho.cotas * produto.preco})
